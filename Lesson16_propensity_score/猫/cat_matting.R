...
dataset <- read_excel("applied/傾向スコアマッチング/猫/catdata.xlsx")
view(dataset)
...
## 投薬と除去日数との相関を確認
ggplot(dataset, 
       aes(x = factor(treatment), y = days, fill = treatment)) +
  geom_boxplot() +
  labs(title = "Distribution of Days by Treatment",
       x = "Treatment",
       y = "Days") +
...
# 共変量調整をしない解析
mod_naive <- lm(days~ treatment, data = dataset)
...
#背景因子影響をプロット
love.plot(treatment ~ weight + age+ hair_type,
...
#WeightItパッケージ
# 傾向スコアの推定と重みの算出
psmodel_wtit <- weightit(treatment ~ weight + age+ hair_type,
...
# 重み付けの線形回帰
mod_iptw_wtit <- svyglm(days ~ treatment,
